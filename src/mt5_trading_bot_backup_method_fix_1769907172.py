...
class MT5TradingBot:
    ...
    def calculate_indicators(self, df):
        ...
        # DETAILED LOGGING - FORCE EXECUTION
        logging.info("="*80)
        logging.info("🔧 DETAILED INDICATOR CALCULATION STARTING")
        logging.info("="*80)
        logging.info(f"📊 Input Data: {len(df)} bars")
        logging.info(f"📊 Data Range: {df.index[0]} to {df.index[-1]}")
        logging.info(f"📊 Latest Close: {df["close"].iloc[-1]:.5f}")
    ...
    def update_trailing_stop(self, position_ticket, symbol, direction):
        ...
        df = self.calculate_indicators_with_logging(df)
